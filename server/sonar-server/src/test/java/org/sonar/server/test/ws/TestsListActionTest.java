...
public class TestsListActionTest {
  ...
  public void list_based_on_test_uuid() throws Exception {
    ...
    TestDoc test = new TestDoc()
      ...
      .setCoveredFiles(File1.COVERED_FILES)
  ...
  public void list_based_on_test_file_key() throws Exception {
    MockUserSession.set().addComponentPermission(UserRole.CODEVIEWER, "SonarQube", "FILE-KEY");
    ComponentDao componentDao = mock(ComponentDao.class);
    when(dbClient.componentDao()).thenReturn(componentDao);
    when(componentDao.getByKey(any(DbSession.class), eq("FILE-KEY"))).thenReturn(new ComponentDto().setUuid(File1.FILE_UUID));
    when(dbClient.componentDao().getByUuids(any(DbSession.class), anyCollectionOf(String.class))).thenReturn(Arrays.asList(
      new ComponentDto()
        .setUuid(File1.FILE_UUID)
        .setLongName(File1.LONG_NAME)
    ...
    when(testIndex.searchBySourceFileUuidAndLineNumber(eq(mainFileUuid), eq(10), any(SearchOptions.class)).getDocs()).thenReturn(
      Arrays.asList(
        new TestDoc()
          ...
          .setStatus(File1.STATUS)
          .setMessage(File1.MESSAGE)
          .setCoveredFiles(new ArrayList<CoveredFileDoc>())
          .setStackTrace(File1.STACKTRACE),
        new TestDoc()
          .setUuid(File2.UUID)
          .setName(File2.NAME)
          ...
          .setCoveredFiles(File2.COVERED_FILES)
  ...
  private static final class File1 {
    ...
    public static final List<CoveredFileDoc> COVERED_FILES = Arrays.asList(new CoveredFileDoc().setFileUuid("MAIN-FILE-UUID-1").setCoveredLines(Arrays.asList(1, 2, 3)));
  ...
  private static final class File2 {
    ...
    public static final List<CoveredFileDoc> COVERED_FILES = Arrays.asList(new CoveredFileDoc().setFileUuid("MAIN-FILE-UUID-1").setCoveredLines(Arrays.asList(11, 12, 13)));
